...
class DataPreProcessPipeline(object):
    ...
    def pp_add_prefix_to_cols(self, prefix: str, columns: List[str]) -> DataFrame:
        return self.df.select([F.col(c).alias(f"{prefix}_{c}") for c in columns])
    
    def pp_prepared_title_array(self, title: str) -> DataFrame:
